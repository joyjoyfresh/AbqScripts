...
SOURCE_FILE_NAMES = [  # 定义需要复制到每个模型目录的源文件名列表
    ...
    "Distribution_PGA_v5.py",  # 定义分布后处理脚本文件名
]  # 结束源文件名列表定义
...
PREFERRED_RELATIVE_PATHS = {  # 定义各源文件的优先相对路径列表
    ...
    "Distribution_PGA_v5.py": [os.path.join("Postprocess", "Distribution_PGA_v5.py")],  # 定义分布后处理脚本优先路径
    "Postprocess_PGA_v6.py": [os.path.join("Postprocess", "Postprocess_PGA_v6.py")],  # 定义 PGA 后处理脚本优先路径
}  # 结束优先相对路径定义

RUN_ORDER = ["VAB_oblique_noGUI_v13.py", "Postprocess_PGA_v6.py", "Distribution_PGA_v5.py"]  # 定义每个文件夹内脚本执行顺序
