# -*- coding: utf-8 -*-
"""将已通过 F0/V1/F1 的证据写入第三章活动 Word 工作稿。"""
...
DOCX = os.path.join(ROOT, 'docs', '论文材料', '边坡地震动放大效应研究论文初稿（第三章重构）.docx')
F1_FIG = os.path.join(ROOT, 'Run', 'ch3_F1_frequency_theory', 'run-003', 'F1_A1_transfer_function.png')


def insert_paragraph_after(paragraph, text='', style='Body Text CN'):
...
def insert_picture_after(paragraph, path, width=Inches(5.8)):
    result = insert_paragraph_after(paragraph, style='Body Text CN')
    result.alignment = 1
    result.add_run().add_picture(path, width=width)
    return result
...
def find_heading(doc, text):
    for paragraph in doc.paragraphs:
        if paragraph.text.strip() == text:
            return paragraph
    raise RuntimeError('找不到章节标题: %s' % text)
...
CONTENT = {
    ...
    '3.2 控制方程、基本假定与模型范围': [
        '有限元模型求解二维弹性动力学方程 \(\\rho\\ddot{\\boldsymbol u}=\\nabla\\cdot\\boldsymbol\\sigma+\\boldsymbol b\\)，其中 \(\\boldsymbol u\) 为位移，\(\\boldsymbol\\sigma=\\boldsymbol C: \\boldsymbol\\varepsilon\)，应变由小变形关系给出。材料采用各向同性线弹性本构，剪切波速、密度和泊松比由配置文件注入，纵波速由弹性参数换算得到。',
        '输入波采用有限时长 Ricker 加速度记录。斜入射通过水平慢度 \(p=\\sin\\theta/V_s\) 引入传播相位，人工边界由弹簧—阻尼器和自由场等效节点力共同实现。独立参考解不调用生产有限差分自由场内核，而由 `reference_layered_psv_v1.py` 计算均质 P–SV 或成层传递函数；垂直入射成层退化另采用独立 SH 层状传递公式。',
    ...
    '3.3.4 阻尼、观测坐标与响应指标': [
        ...
        '地震动放大采用输入—输出传递函数 \(H_q(f)=U_q^{\\rm surface}(f)/U^{\\rm input}(f)\)。F1 的主指标为目标频带幅值 NRMSE、目标频率增益相对误差和合成 P–SV 传递谱峰频误差；竖向单分量峰频在斜入射成层问题中只作为辅助诊断。'
    ],
    ...
    '3.4.3 误差指标、事前门槛与误差预算': [
        'F1 的频带定义为 \([0.5f_c,1.5f_c]\)。幅值传递函数 NRMSE 和目标频率增益误差的事前门槛均为5%，合成 P–SV 传递谱主频相对误差门槛为3%。完整记录窗使用矩形窗；为减少离散频点量化误差，峰频估计采用8倍零填充，但不改变原始时程或幅值指标。',
    ...
    '3.5.2 均质半空间解析解对比': [
        ...
        ('figure', F1_FIG),
        '图中三条内部测点传递函数曲线围绕理论自由表面系数变化，空间差异很小。该结论仅支撑平场均质输入与人工边界实现，不等同于二维坡地散射验证。'
    ],
    '3.5.3 平坦成层场地独立一维对比': [
        ...
        'A5 的竖向单分量峰频出现局部峰分裂，因此主频采用合成传递谱 \(\\sqrt{|H_h|^2+|H_v|^2}\) 评价，三点合成峰频误差均为0；竖向单分量峰频只作为辅助诊断。A4、A5 共同表明，当前数据链能够处理层间反射和 P–SV 分量转换，但尚不能替代 V3 计划中的更大独立一维矩阵。'
    ],
...
def main():
    doc = Document(DOCX)
    # 删除旧占位正文，保留标题和目录结构。
    headings = [p for p in doc.paragraphs if p.text.strip().startswith('3.')]
    start = next(i for i, p in enumerate(doc.paragraphs) if p.text.strip() == '3.1 研究目标、验证问题与适用边界')
    end = next(i for i, p in enumerate(doc.paragraphs) if p.text.strip() == '3.9 本章小结')
    for paragraph in list(doc.paragraphs[start + 1:end]):
        if paragraph.style.name not in ('Heading 2', 'Heading 3'):
            paragraph._element.getparent().remove(paragraph._element)
    for heading_text, paragraphs in CONTENT.items():
        heading = find_heading(doc, heading_text)
        cursor = heading
        for item in paragraphs:
            if isinstance(item, tuple) and item[0] == 'figure':
                if os.path.isfile(item[1]):
                    cursor = insert_picture_after(cursor, item[1])
            else:
                cursor = insert_paragraph_after(cursor, item)
    doc.save(DOCX)
    print('第三章内容已写入: %s' % DOCX)
