...
SCRIPT_SEQUENCE = [  # 定义脚本顺序配置列表
    {"path": r"C:\Users\12462\Documents\Code\AbqScripts\Modeling\Multi\VAB_oblique_TAF_double_v4.py", "parameter_target": True},  # 定义建模脚本完整路径与是否作为参数替换目标
...
def main():  # 定义主函数
    ...
    def process_folder(item):
        folder_name, params = item
        folder_path = os.path.join(root_dir, folder_name)
        print("\n==============================")  # 输出分隔线便于阅读日志
        print("开始处理文件夹：{}".format(folder_path))  # 输出当前处理文件夹信息
    ...
    # 使用线程池并发运行，最大并发数设为 2（每个文件夹使用 4 CPUs，刚好占满 8 个物理核心）
    max_workers = 2
    print("开始并行批处理，最大并发任务数：{}".format(max_workers))
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        results = executor.map(process_folder, folder_plan)
        for folder_path, ok in results:
            if not ok:
