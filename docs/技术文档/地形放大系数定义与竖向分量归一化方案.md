...
从 ODB 提取上述指标必须在 Abaqus CAE 内核运行：`abaqus cae noGUI=Postprocess_All_surface_v2.py`。普通 Python 只用于已有 `surface_response_*.csv` 或 `sgrid_response_*.csv` 的重采样和重绘；当目录中没有这些 CSV 时，脚本会以非零状态退出并提示正确命令，避免静默地产生空结果。
