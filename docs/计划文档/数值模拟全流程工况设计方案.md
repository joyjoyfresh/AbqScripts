...
| Version Label | `ch4_ch6_case_plan_20260717` |
...
- 从[Autorun_template_v2.py](../../Batch/Autorun_template_v2.py)派生正式`test/Batch/Autorun_ch4_ch6_v1.py`，但不得直接把当前模板当作生产入口
...
| 第4章内容 | 使用工况 | 核心输出 |
| --- | --- | --- |
| 均质坡方向效应 | H1 | `H_topo(xi_f,s)`图谱、峰值与峰位 |
...
| 独立确认 | L3 | 分区图确认率与误判工况 |
| 多层与强度边界 | M0、N0 | 结论适用范围 |
| 时域闭环 | W0 | PGA、PGV、Sa、Arias和峰位 |
...
| 字段 | 作用 |
| --- | --- |
| `case_id`、`pool_id`、`split` | 唯一身份和数据角色 |
...
- 原始Abaqus工况与产物放入`test/Abaqus/ch4_ch6/<pool>/<case_id>/`
- 正式启动入口放入`test/Batch/Autorun_ch4_ch6_v1.py`，由普通Python接收工况根目录
...
- 规范数据集以复频响NPZ、质量JSON、元数据和主清单为准，CSV幅值表仅用于人工检查
...
_维护状态：设计完成，等待G0生产链修复与G1频响闸门执行。_
