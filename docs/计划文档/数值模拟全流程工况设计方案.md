...
当前[Postprocess_All_surface_v2.py](../../Postprocess/Postprocess_All_surface_v2.py)的能力边界为：`compute_H`只返回`|FFT(a_out)|/|FFT(a_in)|`，本质上已经是`FSAF_inc`，但不是复频响；现有`H_topo_h`是同侧远场参考台站谱比，也不等同于本方案要求的匹配均质坡`H_topo`；脚本尚未输出相位、显式`valid_mask`或反应谱。好处是`surface_results.npz`已经保存逐节点水平/竖向完整时程和输入时程，后续增加5%阻尼反应谱与`RSAF`无需重新读取或重跑既有ODB。
...
- 后处理升级为复频响、相位、有效频带掩码、`FSAF`派生视图、5%阻尼反应谱与质量JSON；原`H_topo_h`更名为参考台站谱比，避免与匹配均质坡`H_topo`混淆
