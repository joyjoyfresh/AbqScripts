...
def select_cases(package):
    ...
    for thickness_ratio in THICKNESS_RATIOS:
        ...
        if matched.size != 1:
            raise RuntimeError("厚度比 %.2f 未唯一匹配到工况" % thickness_ratio)
...
def main():
    ...
    letters = "abcd"
    mesh = None
    for order, (ax, case_id, thickness_ratio) in enumerate(
        zip(heat_axes, case_ids, THICKNESS_RATIOS)
    ):
        ...
        ax.set_title(
            "(%s) $d/h$=%.2f（%s）" % (letters[order], thickness_ratio, case_id),
            pad=6,
        )
    ...
    for order, thickness_ratio in enumerate(THICKNESS_RATIOS):
