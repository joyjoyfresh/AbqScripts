...
def main():
    ...
    logger = log_step('VAB_oblique_TAF_double_v2.log')  # 初始化日志并写入当前版本日志文件
    ...
    try:
        ...
        mesh_size = 0.04 * h  # 把模型网格尺寸设为高度h的4%
...
def find_acc_txt(logger=None):
    """
    查找当前工作目录下所有 .txt 文件，并读取每个加速度文件的分析步时长 and 增量步。
    """
    cwd = os.getcwd()
...
def create_flat_model(total_L, H_flat,
    ...
    if total_L <= 0:
        raise ValueError('total_L 必须 > 0')
    if H_flat <= 0:
        raise ValueError('H_flat 必须 > 0')
    if H_lower is None:
        raise ValueError('H_lower 必须提供以便进行匹配的水平切割')
    ...
    # ============ 材料与截面 ============
    mat1_params = _compute_material_params(cs1, vv1, density1)
    mat2_params = _compute_material_params(cs2, vv2, density2)

    mat1_name = _next_available_name('Material-L1', model.materials)
    mat1 = model.Material(name=mat1_name)
    mat1.Elastic(table=((mat1_params['EE'], vv1),))
    mat1.Density(table=((density1,),))

    mat2_name = _next_available_name('Material-L2', model.materials)
    mat2 = model.Material(name=mat2_name)
    mat2.Elastic(table=((mat2_params['EE'], vv2),))
    mat2.Density(table=((density2,),))
    log_step(logger, '%s 双层材料已定义: %s, %s', model_name, mat1_name, mat2_name)

    sec1_name = _next_available_name('Section-L1', model.sections)
    model.HomogeneousSolidSection(name=sec1_name, material=mat1_name, thickness=1.0)
    sec2_name = _next_available_name('Section-L2', model.sections)
    model.HomogeneousSolidSection(name=sec2_name, material=mat2_name, thickness=1.0)
    log_step(logger, '%s 双层截面已创建: %s, %s', model_name, sec1_name, sec2_name)
    ...
    part_faces = part.faces
    partition_sketch = model.ConstrainedSketch(
        name='__flat_partition__', sheetSize=max(total_L, H_flat) * 2
    )
    partition_sketch.Line(point1=(total_L, H_lower), point2=(0.0, H_lower))
    part.PartitionFaceBySketch(faces=part_faces, sketch=partition_sketch)
    del model.sketches['__flat_partition__']
    log_step(logger, '%s 网格前水平切割完成: H_lower=%.3f', model_name, H_lower)
    ...
    for face in part.faces:
        ...
        if yc < interface_h:
            layer1_faces.append(face)
        else:
            layer2_faces.append(face)
    ...
    if len(layer1_faces) > 0:
        layer1_face_seq = _to_face_sequence(layer1_faces)
        part.SectionAssignment(region=Region(faces=layer1_face_seq), sectionName=sec1_name, offset=0.0, offsetType=MIDDLE_SURFACE, offsetField='', thicknessAssignment=FROM_SECTION)
    if len(layer2_faces) > 0:
        layer2_face_seq = _to_face_sequence(layer2_faces)
        part.SectionAssignment(region=Region(faces=layer2_face_seq), sectionName=sec2_name, offset=0.0, offsetType=MIDDLE_SURFACE, offsetField='', thicknessAssignment=FROM_SECTION)
    log_step(logger, '%s 截面分配完成: interface_h=%.3f, L1面=%d, L2面=%d', model_name, interface_h, len(layer1_faces), len(layer2_faces))
