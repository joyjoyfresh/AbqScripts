...
damping_cfg = {                 # 材料阻尼配置（对齐论文 Q 衰减）
    ...
    'fc': None,                 # 输入波主频(Hz)：None=从加速度记录自动估计；可显式指定
...
def _estimate_dominant_freq(acc, dt):  # 定义主频估计函数
    """对加速度时程做 FFT，返回幅值谱最大处频率（DC 置零），作为输入波主频 fc 的估计。"""  # 说明函数用途
    acc_centered = acc - np.mean(acc)  # 去除直流分量
    N = len(acc_centered)  # 信号长度
    freq = np.fft.rfftfreq(N, d=dt)  # 正频率轴
    spec = np.abs(np.fft.rfft(acc_centered))  # 幅值谱
    spec[0] = 0.0  # 直流分量置零
    idx_max = np.argmax(spec)  # 幅值谱最大值索引
    return float(freq[idx_max])  # 返回主频估计值（Hz）


def _damping_ratio_from_q(cs, is_bedrock, dcfg):  # 定义品质因子→阻尼比换算函数
    """根据剪切波速和是否为基岩计算品质因子 Q 与阻尼比 ξ = 1/(2Q)。
    ...
    """  # 说明函数用途与参数
    if is_bedrock:  # 基岩层
        Q = dcfg['q_bedrock']  # 基岩品质因子（≈无衰减）
    else:  # 覆盖层
        Q = dcfg['qs_factor'] * cs  # 论文 coarse-grain 法：Qs = 0.05·cs
    xi = 1.0 / (2.0 * Q)  # 阻尼比 ξ = 1/(2Q)
    return Q, xi  # 返回品质因子与阻尼比


def _rayleigh_coeffs(xi, dcfg, fc):  # 定义瑞利阻尼系数计算函数
    """根据阻尼比 ξ 和配置计算瑞利阻尼系数 (alpha, beta)。
    ...
    if dcfg['method'] == 'stiffness':  # 仅刚度比例阻尼
        alpha = 0.0  # 质量比例系数为零
        beta = xi / (math.pi * fc)  # 刚度比例系数：ξ = β·π·fc
    else:  # 瑞利双频拟合法（默认）
        w1 = 2.0 * math.pi * dcfg['f1_factor'] * fc  # 下限圆频率
        w2 = 2.0 * math.pi * dcfg['f2_factor'] * fc  # 上限圆频率
        alpha = 2.0 * xi * w1 * w2 / (w1 + w2)  # 质量比例系数
        beta = 2.0 * xi / (w1 + w2)  # 刚度比例系数
    return alpha, beta  # 返回瑞利阻尼系数


def _resolve_damping(dcfg, fc_est):  # 定义阻尼配置解析函数
    """解析阻尼配置：若 fc 未显式指定则用估计值填充，返回解析后的配置副本。"""  # 说明函数用途
    resolved = dict(dcfg)  # 浅拷贝配置字典
...
def create_model(site, geom, mesh_size, cae_name=None, logger=None, damping=None):
    ...
    damping: 解析后的阻尼配置 dict（含 fc），为 None 则不施加材料阻尼
    """  # 说明函数用途与参数
    ...
    if damping and damping['enable']:  # 判断是否需要施加材料阻尼
        Q_b, xi_b = _damping_ratio_from_q(site.bedrock.cs, True, damping)  # 计算基岩品质因子与阻尼比
        alpha_b, beta_b = _rayleigh_coeffs(xi_b, damping, damping['fc'])  # 计算基岩瑞利阻尼系数
        mat_bedrock.Damping(alpha=alpha_b, beta=beta_b)  # 施加基岩瑞利阻尼
    ...
    if damping and damping['enable']:  # 判断是否需要施加材料阻尼
        Q_o, xi_o = _damping_ratio_from_q(site.overlying.cs, False, damping)  # 计算覆盖层品质因子与阻尼比
        alpha_o, beta_o = _rayleigh_coeffs(xi_o, damping, damping['fc'])  # 计算覆盖层瑞利阻尼系数
        mat_overlying.Damping(alpha=alpha_o, beta=beta_o)  # 施加覆盖层瑞利阻尼
...
def build_models(acc_info, base_model, part_name, inst_name,
    ...
    for acc_file, tp, inc in acc_info:  # 遍历每个加速度记录
        ...
        model.ImplicitDynamicsStep(  # 创建隐式动力分析步
            ...
            nlgeom=OFF, application=TRANSIENT_FIDELITY)  # 关闭几何非线性，TRANSIENT_FIDELITY 让物理阻尼主导
...
def main():
    ...
    global damping_cfg  # 声明全局阻尼配置变量以便 case_config 注入覆盖
    ...
    try:
        ...
        # ============ 估计输入波主频并解析阻尼配置 ============
        if damping_cfg['fc'] is None:  # 未显式指定主频时自动估计
            acc_sample = np.loadtxt(acc_info[0][0])  # 读取第一条加速度记录
            dt_sample = acc_sample[1, 0] - acc_sample[0, 0]  # 提取时间步长
            fc_est = _estimate_dominant_freq(acc_sample[:, 1], dt_sample)  # 估计主频
            log_step(logger, '自动估计输入波主频 fc = %.3f Hz', fc_est)  # 记录估计结果
        else:  # 已显式指定主频
            fc_est = damping_cfg['fc']  # 直接使用指定值
        damping = _resolve_damping(damping_cfg, fc_est)  # 解析阻尼配置（填充 fc）
        log_step(logger, '材料阻尼配置: enable=%s, method=%s, fc=%.3f Hz',  # 记录阻尼配置
                 damping['enable'], damping['method'], damping['fc'])  # 输出关键参数
        ...
        flat_base_model, flat_part_name, flat_inst_name = create_flat_model(  # 创建平坦自由场基础模型
            ...
            damping=damping)  # 传入材料阻尼配置（与斜坡模型一致，保证 TAF 分母可比）
