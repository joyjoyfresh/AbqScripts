...
def build_models(acc_info, base_model, part_name, inst_name, angle, cs, vv, density, 
    ...
    for acc_file, tp, inc in acc_info:
        ...
        # 为 TOP_SURFACE 节点集创建加速度场输出（频率=1，每帧输出）
        if 'TOP_SURFACE' in model.parts[part_name].sets:
            if 'TOP_SURFACE' in inst_sets:
                model.FieldOutputRequest(
                    name='F-Output-TOP-SURFACE',
                    createStepName=step_name,
                    variables=('A',),
                    frequency=1,
                    region=inst_sets['TOP_SURFACE'])
                log_step(logger, '%s 已为 TOP_SURFACE 创建加速度场输出', new_model_name)
