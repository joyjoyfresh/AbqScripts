...
def create_model(total_L, h, i, cs, vv, density, mesh_size,
    ...
    # ============ 创建顶面节点集（用于后处理 PGA） ============
    # 与左/右/底边界一致，采用“节点坐标 + 容差”筛选。
    top_surface_labels_set = set()
    for node in part.nodes:
        x = node.coordinates[0]
        y = node.coordinates[1]

        # 左平台顶面: y = H_upper, x <= left_flat
        if x <= left_flat + tol and abs(y - H_upper) < tol:
            top_surface_labels_set.add(node.label)
            continue

        # 斜坡面: y = H_upper - (x-left_flat)*h/w_slope
        ...
        # 右平台顶面: y = H_lower, x >= left_flat + w_slope
        if x >= left_flat + w_slope - tol and abs(y - H_lower) < tol:
            top_surface_labels_set.add(node.label)

    # 排除与左/右/底边界重叠的拐角节点，保证 TOP_SURFACE 纯顶面
    excluded_labels = set(l_labels) | set(r_labels) | set(b_labels)
    top_surface_labels = tuple(sorted(top_surface_labels_set - excluded_labels))
