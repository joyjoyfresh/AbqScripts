...
material_cfg = {  # 定义材料参数配置
    ...
    #   1 项 [overlying]    → 双层模型（基岩 + 覆盖层，等同 v4 现状）
    #   2 项 [surface, ...] → 三层模型（表层 V_S1 + 覆盖层 V_S2 + 基岩，论文模型）
    # 每层用相对基岩的 velocity_ratio（V_R/V_S）+ 泊松比 + 密度描述；
    # 除最底有限层（覆盖层）外，其余层必须给定固定 thickness（m）；
    # 最底有限层不写 thickness，其厚度由斜坡几何 H 决定（填充至顶部固定层之下）。
    'layers': [  # 定义基岩之上的有限层（从上到下）
        {'name': 'surface', 'velocity_ratio': 1.6, 'poisson_ratio': 0.3,  # 表层 V_S1：相对基岩波速比与泊松比
         'density': 2500, 'thickness': 50.0},  # 表层密度与固定厚度 h1 (m)
        {'name': 'overlying', 'velocity_ratio': 1.25, 'poisson_ratio': 0.3,  # 覆盖层 V_S2：相对基岩波速比与泊松比
