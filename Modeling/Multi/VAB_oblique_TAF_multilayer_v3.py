...
# ==========================================================
#  【本版 = 双层验证版：配置严格对齐 VAB_oblique_TAF_double_v4.py】
#  目的：用 multilayer 引擎（layers 仅一层覆盖层，即 M=1 双层）复刻 double_v4 的同一模型，
#       验证 multilayer_v* 与 double_v4 的 Abaqus 运行结果是否一致。
#  对齐项（与 double_v4 完全相同）：
#    angle=15；基岩 E=26 GPa, ν=0.3, ρ=2500（Vr=2000 m/s）；
#    覆盖层 velocity_ratio=Vr/Vs=1.25（Vs=1600）, ν=0.3, ρ=2500；无表层（双层）；
#    几何 i=45, h/H=0.5, H−h=200, total_L=1800, left_flat=1000, bedrock_thickness=200；mesh=4。
#  说明：layers 只含 overlying（最底覆盖层，厚度由几何决定）→ M=1，
#       自由场引擎在 M=1 时已逐节点退化为 double_v4（见 test/test_degenerate_vs_v4.py，差~1e-11），
#       网格亦为结构化（无界面切坡），故理应得到与 double_v4 一致的结果。
# ==========================================================
material_cfg = {  # 定义材料参数配置（双层，对齐 double_v4）
