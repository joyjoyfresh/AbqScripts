...
# ==========================================================
#  【本版 = v3：在 v2 基础上新增材料阻尼（对齐 Shen 等 2025 的 Q 衰减）与降低数值阻尼】
#  引擎（射线法自由场 + VAB 等效力）与 v2 完全一致，未改动；本版仅在【FE 求解端】增三处：
#    ①各层施加瑞利材料阻尼（coarse-grain 法 Qs=qs_factor·cs、基岩 Q≈999）；
#    ②分析步数值积分由 MODERATE_DISSIPATION 改 TRANSIENT_FIDELITY（让物理阻尼主导）；
#    ③阻尼参数随 case_config.json 注入、并写入 case_meta.json（可复现）。
#  目的：复现论文图15（无阻尼会高估分层共振，使 TAF 峰值偏高、曲线过振）。
#  引擎与 v2 完全相同（自由场支持 1/2/3… 层）；沿用 v2 的【配置注入】：
...
def _load_case_config(material_cfg, geometry_cfg, mesh_size, damping_cfg, logger):  # 加载工况配置注入
    ...
    case_config.json 结构（各键可选，缺省即用默认）：
      {"material_cfg": {...}, "geometry_cfg": {...}, "mesh_size": 4, "damping_cfg": {...部分或全部...}}
    ...
    try:  # 尝试读取并覆盖
        ...
        if logger:  # 记录成功
            log_step(logger, '已加载 case_config.json 覆盖默认配置: 入射角=%s, 层数(有限层)=%d, i=%s, 阻尼=%s/%s',  # 输出关键覆盖项
                     material_cfg.get('angle'), len(material_cfg.get('layers', [])), geometry_cfg.get('i'),
                     damping_cfg.get('enable'), damping_cfg.get('method'))  # 输出阻尼开关与方法
...
def main():
    ...
    try:
        ...
        material_cfg, geometry_cfg, mesh_size, damping_cfg = _load_case_config(  # 加载并覆盖（含阻尼）
        ...
        log_step(logger, '材料阻尼: enable=%s, method=%s, fc=%s',  # 记录最终阻尼设定
                 damping.get('enable'), damping.get('method'), damping.get('fc'))  # 输出开关/方法/主频

        _write_case_meta(material_cfg, geom, site, mesh_size, _script_name(), logger, damping=damping)  # 写出统一工况元数据 case_meta.json（含阻尼块；脚本名不依赖 __file__）
        ...
        base_model, part_name, inst_name = create_model(  # 创建斜坡基础几何与网格模型
            site=site, geom=geom, mesh_size=mesh_size, cae_name=cae_name, logger=logger, damping=damping)  # 传入场地/几何/网格/文件名/阻尼

        flat_base_model, flat_part_name, flat_inst_name = create_flat_model(  # 创建平坦自由场基础模型
            site=site, geom=geom, mesh_size=mesh_size, logger=logger, damping=damping)  # 传入场地/几何/网格/阻尼（与斜坡同参）
