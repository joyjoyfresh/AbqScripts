...
# ==========================================================
#  【本版 = v5：在 v4 基础上进行四项对齐 Shen 等(2025) 论文的改进】
#
#  ① 自由场"层内材料/角度一致化"（最高优先，修复 v4 最大物理错误）：
#       v4 对所有边界节点的应力/投影公式均使用基岩标量与基岩入射角，
#       与已按本层取值的弹簧/阻尼系数自相矛盾；软表层 Vs1=400 的侧边节点
#       面力被高估约 5×（ρcs_bedrock/ρcs_surface）。
#       v5 对有限层侧边节点改用本层 alpha/beta 与本层 GG/cs/lam/cp，
#       使面力计算与弹簧/阻尼口径一致。
#       等效反射幅值 Rss_eff/Rsp_eff 仍基于射线法（近似），不声称修好界面透射幅值。
#
#  ② 网格自适应（Kuhlemeyer-Lysmer 准则 Δl≤cs_min/(10·fmax)）：
#       新增 mesh_cfg 配置块与 _max_element_size() 函数；
#       main() 中自动计算 mesh_used=min(mesh_size, Δl_max)，可被 case_config.json 覆盖。
#
#  ③ 时间步充分性校验护栏：
#       新增 time_cfg 配置块；读取输入记录后检查 steps/周期 是否满足最低要求；
#       不满足时 np.interp 重采样（当前 dt=1e-3 均达标，不触发重采样，仅护栏）。
#
#  ④ 衰减分离与已知局限文档化：
#       模块注释明确列出 Rayleigh 无法分离 Qp/Qs、界面 SV↔P=0、CPE4 线性
#       等与论文 SPECFEM2D 的已知差异。MAX_REFLECT_ORDER 可通过 case_config.json 注入。
#
#  ▼ 已知局限（方法差异，不影响 SV 主导下的 TAF 分析，但撰写方法节须注明）：
#    (a) Rayleigh 阻尼 α/β 对 P/S 同一阻尼比，无法独立实现论文 Qp=2Qs；
#        本版仅按 Qs=qs_factor·cs 施加，SV 主导可接受。
#    (b) 界面 SV↔P 转换系数取 Rsp=Tsp=0（射线法近似，见 _compute_interface_sv_coeff）；
#        多层为独立腔截断近似（忽略跨腔耦合）。
#    (c) 单元为 CPE4 线性（+楔形 CPE3），对比论文 SPECFEM2D 谱元高阶；
#        靠项② 网格加密缓解数值频散。
#    (d) 弹簧刚度参考长度取固定 R=ymax，系数 0.5/0.25（Liu 2006 标准取值）。
#    (e) 项① 残留的界面透射幅值射线近似（等效反射系数仍用阻抗近似层栈递归）。
#
#  引擎：射线法（时域到时延迟叠加），与 v4 完全一致，不换传播矩阵。
...
_DEFAULT_SCRIPT_NAME = 'VAB_oblique_TAF_multilayer_v5.py'  # 本脚本已知文件名（__file__ 缺失时的兜底）
...
def build_models(acc_info, base_model, part_name, inst_name,
    ...
    for acc_file, tp, inc in acc_info:  # 遍历每个加速度记录
        ...
        VAB_oblique(site=site, geom=geom, angle=angle,  # 调用人工边界构建函数（传入场地与几何对象）
                    ...
                    tcfg=tcfg, fc_used=fc_used)  # 项③：时间步校验参数
...
def _load_case_config(material_cfg, geometry_cfg, mesh_size, damping_cfg, logger,
    ...
    v5 新增支持 mesh_cfg / time_cfg / max_reflect_order 三类注入（项②③④）。
    case_config.json 结构（各键可选，缺省即用默认）：
      {
        "material_cfg": {...}, "geometry_cfg": {...}, "mesh_size": 4,
        "damping_cfg": {...}, "mesh_cfg": {...}, "time_cfg": {...},
        "max_reflect_order": 3
      }
    返回覆盖后的 (material_cfg, geometry_cfg, mesh_size, damping_cfg, mesh_cfg, time_cfg, max_reflect_order)。
    无文件或解析失败则原样返回默认。
    """
    ...
    if not os.path.isfile(path):  # 无注入文件 → 用默认配置单独运行
        ...
        return material_cfg, geometry_cfg, mesh_size, damping_cfg, mesh_cfg_out, time_cfg_out, max_reflect_order_out  # 原样返回默认
    try:  # 尝试读取并覆盖
        ...
        if logger:  # 记录成功
            log_step(logger, '已加载 case_config.json 覆盖默认配置: 入射角=%s, 层数(有限层)=%d, i=%s, 阻尼=%s/%s, mesh_auto=%s, order=%s',  # 输出关键覆盖项
                     material_cfg.get('angle'), len(material_cfg.get('layers', [])), geometry_cfg.get('i'),
                     damping_cfg.get('enable'), damping_cfg.get('method'),
                     mesh_cfg_out.get('auto'), max_reflect_order_out)  # 输出网格/反射阶数
    except Exception as _e:  # 解析失败
        if logger:  # 记录告警
            log_step(logger, '加载 case_config.json 失败(改用默认配置): %s', str(_e))  # 输出失败告警
    return material_cfg, geometry_cfg, mesh_size, damping_cfg, mesh_cfg_out, time_cfg_out, max_reflect_order_out  # 返回（可能被覆盖的）配置
...
def _write_case_meta(material_cfg, geom, site, mesh_size, script_name, logger, damping=None):  # 写出统一工况元数据
...
def main():
    ...
    try:
        log_step(logger, '脚本开始执行 (VAB_oblique_TAF_multilayer_v5)')  # 写入脚本启动日志

        # 配置注入：若工况文件夹有 case_config.json 则覆盖默认配置（支持任意变参数/层数/阻尼/网格/时间步/反射阶数）
        (material_cfg, geometry_cfg, mesh_size, damping_cfg,
         _mesh_cfg, _time_cfg, _max_reflect_order) = _load_case_config(  # 加载并覆盖（v5 新增网格/时间/阶数注入）
            material_cfg, geometry_cfg, mesh_size, damping_cfg, logger)  # 传入默认配置与日志器
        MAX_REFLECT_ORDER = _max_reflect_order  # 全局更新反射截断阶数（项④）
        ...
        log_step(logger, '材料阻尼: enable=%s, method=%s, fc=%s',  # 记录最终阻尼设定
                 damping.get('enable'), damping.get('method'), fc_resolved)  # 输出开关/方法/主频
        ...
        _write_case_meta(material_cfg, geom, site, mesh_used, _script_name(), logger, damping=damping)  # 写出统一工况元数据 case_meta.json（含阻尼块；脚本名不依赖 __file__）
        ...
        base_model, part_name, inst_name = create_model(  # 创建斜坡基础几何与网格模型
            site=site, geom=geom, mesh_size=mesh_used, cae_name=cae_name, logger=logger, damping=damping)  # 传入场地/几何/自适应网格/文件名/阻尼

        flat_base_model, flat_part_name, flat_inst_name = create_flat_model(  # 创建平坦自由场基础模型
            site=site, geom=geom, mesh_size=mesh_used, logger=logger, damping=damping)  # 传入场地/几何/自适应网格/阻尼（与斜坡同参）
        ...
        slope_model_names = build_models(  # 批量复制斜坡模型并施加等效边界
            ...
            tcfg=_time_cfg, fc_used=fc_resolved)  # 项③：时间步校验配置与主频
