...
# ==========================================================
#  脚本说明：本脚本 = VAB_oblique_TAF_multilayer(v8/v9.1 谱系) 去掉平坦对照模型(flat) + 新增土体非线性(EQL)。
#           线性建模 / 自由场 / 网格 / 阻尼等机制沿用 v9.1；土体非线性见下方机制(7)。
#
#  一、 脚本主要用法 (Usage)
...
#       abaqus cae noGUI=VAB_oblique_multilayer_nonlinear_v2.py
#
...
#    7. 土体非线性：等效线性 EQL (eql_cfg；默认 enable=False 即线性=原行为)：
#       - ① 1D 模式 (mode='1d'，默认)：上平台自由场柱按 SHAKE 应变相容迭代——降软层 Vs、增 ξ，
#         再喂给 2D FE 与斜入射自由场(同口径)；经验曲线可切换(darendeli/seed_idriss_sand/vucetic_dobry)；
#         结果(各非线性层 γ_eff/G_Gmax/Vs0→Vs/ξ)写入 case_meta['eql']。
#       - ② 逐单元 2D 模式 (mode='2d_element'，重型，需 Abaqus 实测)：从 ODB 逐单元读最大剪应变、
#         按应变分箱更新 G/ξ、迭代重跑 2D FE，抓坡顶局部软化；建模自动对软层单元请求应变场 E。
#       - 强度由输入波幅值决定 (输入=基岩入射 E、露头=2E)；扫描=按不同幅值的输入多跑几次。
# ==========================================================
...
_DEFAULT_SCRIPT_NAME = 'VAB_oblique_multilayer_nonlinear_v2.py'  # __file__ 缺失时的兜底文件名
...
def _eql_bins_from_strains(geff_by_elem, n_bins):  # 纯函数：按有效剪应变给单元分箱(对数)
    ...
    if hi <= lo * (1.0 + 1e-6):  # 应变近似一致 → 单箱
        ...
        edges = np.logspace(math.log10(lo), math.log10(hi), n_bins + 1)  # 对数等分箱边
    edges[-1] *= 1.0001  # 含右端
    nb = len(edges) - 1
...
def _apply_element_eql_materials(model_name, part_name, soil_elem_labels, geff_by_elem,
    ...
    for bi, (Vs_b, xi_b, gg_b) in enumerate(props):  # 逐箱建材+赋材
        ...
        if not labs:
            continue
        EE = _compute_elastic_modulus_from_wave_speed(Vs_b, vv_soil, rho_soil)  # 由 Vs 反算 E
...
def _run_2d_element_eql(base_model, part_name, inst_name, site, geom, eql_cfg, damping, fc,
                        ffcfg, tcfg, sgeom, run_cfg, acc_rec, angle, job_cfg, logger):
    ...
    for outer in range(n_out):  # 外迭代
        submit_job(num_cpus=job_cfg['num_cpus'], memory_percent=job_cfg['memory_percent'],
                   model_name=model_name, logger=logger)  # 提交并等待
...
def main():
    ...
    try:
        log_step(logger, '脚本开始执行 (VAB_oblique_multilayer_nonlinear_v2)')  # 写入脚本启动日志
