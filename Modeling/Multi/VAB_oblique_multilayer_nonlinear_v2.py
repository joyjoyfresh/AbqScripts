...
def create_model(site, geom, mesh_size, cae_name=None, logger=None, damping=None,
    ...
    if graded:  # 分层非均匀网格——软层细、深部粗，自由四边形平滑过渡
        ...
        elem_code = CPE4R if str(elem_name).upper() == 'CPE4R' else CPE4
...
def _make_boundary_nodes(nodes, sort_axis, ascending, pick_material, ymax, logger=None, model_name='Model-1', boundary_tag='?'):
...
def VAB_oblique(site, geom, angle,
    ...
    nodes_by_boundary = {  # 各边界 -> BoundaryNode 列表
        'l': _make_boundary_nodes(l_nodes, 'y', False, pick_material, ymax, logger, model_name, 'l'),  # 左边界（沿 y 降序）
