...
run_cfg = {                     # v6 新增：运行控制配置（可被 case_config.json 覆盖）
    'run_flat': True,           # 是否建模并求解平坦对照模型（v6 起 TAF 分母为解析值，flat 仅作 QA；量产可设 False 省一半算力）
}                               # 结束运行控制配置
...
_DEFAULT_SCRIPT_NAME = 'VAB_oblique_TAF_multilayer_v6.py'  # 本脚本已知文件名（__file__ 缺失时的兜底）
...
def _fd_input_spectrum(ctx):  # 计算（并缓存）输入波的位移谱与求解频点
    ...
    cached = {'Nfft': Nfft, 'Nout': N, 'dt': dt, 'nfreq': len(freqs),  # 打包缓存内容
...
def _fd_solve_column(column, p, omega, damp_terms, include_damping):  # 柱频域全局矩阵求解
    ...
    try:  # 优先批量直解（numpy 支持堆叠方阵；右端项需补列维以兼容各 numpy 版本）
        ...
        for k in range(nb):  # 逐频点求解
            amps[k] = np.linalg.lstsq(A[k], b[k], rcond=None)[0]  # 最小二乘解
...
def _fd_freefield_at_node(boundary, x0, y0, ymax_col, ctx):  # fd 引擎单节点自由场
    ...
    return {'time': np.asarray(ctx.time_arr, dtype=float), 'ux': out['ux'], 'uy': out['uy'],  # 打包返回
...
def build_models(acc_info, base_model, part_name, inst_name,
                 ...
                 tcfg=None, fc_used=None, ffcfg=None, damping=None):
...
def _write_case_meta(material_cfg, geom, site, mesh_size, script_name, logger, damping=None, ffcfg=None):  # 写出统一工况元数据
...
def main():
    ...
    try:
        log_step(logger, '脚本开始执行 (VAB_oblique_TAF_multilayer_v6)')  # 写入脚本启动日志
        ...
        _write_case_meta(material_cfg, geom, site, mesh_used, _script_name(), logger,  # 写出统一工况元数据 case_meta.json
                         damping=damping, ffcfg=_ff_cfg)  # 含阻尼块与 v6 ff_normalization/freefield 块
