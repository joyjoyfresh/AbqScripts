...
def main():
    ...
    material_cfg = {
        ...
        'overlying': {
            'poisson_ratio': 0.3,
            'density': 2500,
            'velocity_ratio': 1.25,  # VR / Vs 阻抗比，对应 Vs = 2000 / 1.25 = 1600 m/s
        },
...
def create_model(total_L, H_minus_h, i, h_over_H, bedrock_thickness,
    ...
    if sec_assignments['overlying']:
        part.SectionAssignment(region=Region(faces=_to_face_sequence(sec_assignments['overlying'])),
                               ...
                               offsetField='', thicknessAssignment=FROM_SECTION)
    log_step(logger, '%s 截面属性分配完成: Bedrock=%d, Overlying=%d',
             model_name, len(sec_assignments['bedrock']), len(sec_assignments['overlying']))
...
def VAB_oblique(angle,
        ...
        for i in range(n):
            ...
            if boundary == 'l':
                if y0 <= Ly:
                    ...
                    t1 = Ly * np.cos(alpha) / cs + (y0 - Ly) * np.cos(alpha2) / cs2
                    t2 = Ly * np.cos(alpha) / cs + (2 * ymax_col - Ly - y0) * np.cos(alpha2) / cs2
                    t3 = Ly * np.cos(alpha) / cs + (y0 - Ly) * np.cos(beta2) / cp2
                det[i, 1] = t1
                det[i, 2] = t2
                det[i, 3] = t3
                ...
                if y0 <= Ly:
                    ...
                    t2 = Ly * np.cos(alpha) / cs + (2 * ymax_col - Ly - y0) * np.cos(alpha2) / cs2
                    t3 = Ly * np.cos(alpha) / cs + (y0 - Ly) * np.cos(beta2) / cp2
                det[i, 1] = t1 + Lx * np.sin(alpha) / cs
                det[i, 2] = t2 + Lx * np.sin(alpha) / cs
                det[i, 3] = t3 + Lx * np.sin(alpha) / cs
    ...
    det_l = calc_node_delay(node_data_l, 'l', alpha, beta_p, cs, cp, Ly, Lx,
                            cs_overlying, mat_overlying['cp'], alpha2, beta2, ymax_l)
    det_r = calc_node_delay(node_data_r, 'r', alpha, beta_p, cs, cp, Ly, Lx,
                            cs_overlying, mat_overlying['cp'], alpha2, beta2, ymax_r)
    det_b = calc_node_delay(node_data_b, 'b', alpha, beta_p, cs, cp, Ly, Lx,
                            cs_overlying, mat_overlying['cp'], alpha2, beta2, ymax)
    ...
    def batch_add_node_force(node_data, prefix, step_name):
        ...
        n = assembly.instances[instance_name].nodes
...
def submit_job(num_cpus=7, memory_percent=90, model_name='Model-1', logger=None):
    ...
    mdb.Job(name=job_name, model=model_name,
            description='VAB oblique SV-wave analysis (Two-layered slope)',
