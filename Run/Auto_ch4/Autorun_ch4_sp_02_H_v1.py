...
"""小论文批次 02/05：H001—H004 均质基岩坡（全局执行序号 004—007）。

依据《毕业论文第四至第六章与英文期刊小论文融合研究实施总计划最终版》§4.5：
4 个坡角 15°/30°/45°/60°，均质基岩（layers=[]），作为覆盖层修正 Δ_A 的同坡角均质基线。
须在批次 01（V001—V003）数值验证人工审查通过后运行（计划 §8.1 硬停止条件）。

运行形式：
  python Autorun_ch4_sp_02_H_v1.py [求解输出根目录]
缺省输出根目录为 C:\\Users\\12462\\Documents\\Code\\AbqScripts\\Run\\ch4_sp_02_H，
工况文件夹按全局执行顺序命名（case-004-H001 … case-007-H004）。
"""
...
ROOT_DIR = r"C:\Users\12462\Documents\Code\AbqScripts\Run\ch4_sp_02_H"  # 默认求解输出根目录（小论文批次 02/05：H001—H004），可用命令行参数覆盖
...
def base_config(slope_angle, layers, wave_files, extra=None):
    ...
    config = {
        ...
        'run_cfg': {
            ...
            'response_spectrum_cfg': {'enable': False},  # 参数研究阶段不计算反应谱
        },
...
PARAMETER_CASES = [  # H001—H004 均质基岩坡（计划 §4.5，全局执行序号 005—008）
    {"name": "%03d-H%03d" % (5 + k, k + 1),
     "config": base_config(angle, [], [G1B_WAVE])}  # layers=[] 即全基岩坡
    for k, angle in enumerate((15.0, 30.0, 45.0, 60.0))
]
