...
"""小论文批次 04/05：B001—B012 未见组合验证（全局执行序号 073—084）。

依据《毕业论文第四至第六章与英文期刊小论文融合研究实施总计划最终版》§4.7：
12 个未见中间参数组合，用于三变量代理的一次性盲测。
纪律（计划 §4.7/§8.1）：第一次模型锁定前不得求解或读取标签；
本脚本只在全 64 例重拟合并锁模后运行一次，读取标签后不得再修改小论文模型；
小论文发表后也不进入毕业论文五维代理训练。

运行形式：
  python Autorun_ch4_sp_04_B_v1.py [求解输出根目录]
缺省输出根目录为 C:\\Users\\12462\\Documents\\Code\\AbqScripts\\Run\\ch4_sp_04_B，
工况文件夹按全局执行顺序命名（case-072-B001 … case-083-B012）。
"""
...
ROOT_DIR = r"C:\Users\12462\Documents\Code\AbqScripts\Run\ch4_sp_04_B"  # 默认求解输出根目录（小论文批次 04/05：B001—B012），可用命令行参数覆盖
...
def base_config(slope_angle, layers, wave_files, extra=None):
    ...
    config = {
        ...
        'run_cfg': {
            ...
            'response_spectrum_cfg': {'enable': False},  # 未见组合验证阶段不计算反应谱
        },
...
_B_TABLE = [  # (坡角, d/h, rv)：12 个未见中间参数组合（计划 §4.7）
    (22.5, 0.40, 0.375), (22.5, 0.40, 0.675), (22.5, 0.80, 0.525), (22.5, 1.20, 0.375),
    (37.5, 0.40, 0.525), (37.5, 0.80, 0.375), (37.5, 0.80, 0.675), (37.5, 1.20, 0.525),
    (52.5, 0.40, 0.675), (52.5, 0.80, 0.525), (52.5, 1.20, 0.375), (52.5, 1.20, 0.675),
]
PARAMETER_CASES = [  # B001—B012（全局执行序号 073—084）
    {"name": "%03d-B%03d" % (72 + _k, _k),
     "config": base_config(_i, [cover_layer(_rv * 2000.0, _dh * SLOPE_HEIGHT)], [G1B_WAVE])}
    for _k, (_i, _dh, _rv) in enumerate(_B_TABLE, start=1)
]
