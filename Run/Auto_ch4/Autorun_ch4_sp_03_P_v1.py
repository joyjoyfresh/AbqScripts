...
"""小论文批次 03/05：P001—P064 开发工况（全局执行序号 008—071）。

依据《毕业论文第四至第六章与英文期刊小论文融合研究实施总计划最终版》§4.6：
4 坡角 × 4 厚度比 × 4 波速比全因子矩阵，用于三变量幅值频响规律与代理训练。
须在批次 01 数值验证审查完成后运行；本批次数据和独立评价完成后再运行批次 04（计划 §8.1）。

运行形式：
  python Autorun_ch4_sp_03_P_v1.py [求解输出根目录]
缺省输出根目录为 C:\\Users\\12462\\Documents\\Code\\AbqScripts\\Run\\ch4_sp_03_P，
工况文件夹按全局执行顺序命名（case-008-P001 … case-071-P064）。
"""
...
ROOT_DIR = r"C:\Users\12462\Documents\Code\AbqScripts\Run\ch4_sp_03_P"  # 默认求解输出根目录（小论文批次 03/05：P001—P064），可用命令行参数覆盖
...
def base_config(slope_angle, layers, wave_files, extra=None):
    ...
    config = {
        ...
        'run_cfg': {
            ...
            'response_spectrum_cfg': {'enable': False},  # 组合修正阶段不计算反应谱
        },
...
PARAMETER_CASES = []  # P001—P064 开发工况（计划 §4.6，全局执行序号 009—072）
_p = 0
for _i in (15.0, 30.0, 45.0, 60.0):  # 坡角（度）
    for _dh in (0.20, 0.60, 1.00, 1.40):  # 厚度比 d/h
        for _rv in (0.30, 0.45, 0.60, 0.75):  # 波速比 rv
            _p += 1
            PARAMETER_CASES.append({
                "name": "%03d-P%03d" % (8 + _p, _p),
                "config": base_config(_i, [cover_layer(_rv * 2000.0, _dh * SLOPE_HEIGHT)], [G1B_WAVE]),  # 覆盖层 Vs=rv×2000，厚度 d=(d/h)×100 m
            })
