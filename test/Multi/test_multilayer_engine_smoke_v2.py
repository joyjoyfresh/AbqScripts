...
"""多层自由场引擎冒烟测试（针对 v2/v4 版本，不依赖 Abaqus）：验证 1/2/3 层均可构建分层、求等效系数、算自由场。
...
检查项：
  ...
  - 新增的阻尼计算在无 Abaqus 环境下导入正常。
运行：python test/test_multilayer_engine_smoke_v2.py
"""
...
ml = importlib.import_module('VAB_oblique_TAF_multilayer_v2')  # 导入多层 v2 脚本
...
def _check(material_cfg, gcfg, exp_nbands, tag):  # 对一种分层配置做检查
    ...
    print('  [PASS] %s 全部代表节点自由场有限。' % tag)  # 打印通过
