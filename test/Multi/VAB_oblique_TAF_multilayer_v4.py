...
job_cfg = {  # 定义作业参数配置
    'variables': ('U',),  # 设置场输出变量（仅保留位移以降低场输出压力）
    'frequency': 50,  # 设置场输出频率（每50步输出一次）
...
def _create_band_materials_sections(model, strat, damping=None):  # 定义逐带创建材料与截面的函数
    """为分层带（从下到上）逐带创建材料与均质截面，并按配置施加阻尼，返回 [(band, sec_name), ...]。"""  # 说明函数用途
    band_sections = []  # 初始化带-截面映射列表
    for idx, band in enumerate(strat):  # 从下到上遍历每条材料带
        mat = band['mat']  # 取该带材料输入
        ...
        # 若启用阻尼，计算并施加阻尼属性到当前材质层
        if damping and damping.get('enable'):
            is_bedrock = (idx == 0)  # strat[0] 恒为基岩
            Q, xi = _damping_ratio_from_q(mat.cs, is_bedrock, damping)
            alpha, beta = _rayleigh_coeffs(xi, damping, damping['fc'])
            m.Damping(alpha=alpha, beta=beta)  # Abaqus 瑞利阻尼
...
def create_model(site, geom, mesh_size, cae_name=None, damping=None, logger=None):
    ...
    band_sections = _create_band_materials_sections(model, strat, damping=damping)  # 逐带创建材料与截面并施加阻尼
...
def build_models(acc_info, base_model, part_name, inst_name,
    ...
    for acc_file, tp, inc in acc_info:  # 遍历每个加速度记录
        ...
        model.ImplicitDynamicsStep(  # 创建隐式动力分析步
            ...
            nlgeom=OFF, application=TRANSIENT_FIDELITY)  # 关闭几何非线性并降低数值消散
        ...
        if 'H-Output-1' in model.historyOutputRequests:
            model.historyOutputRequests['H-Output-1'].deactivate(step_name)

        # 增加对地表节点集 (TOP_SURFACE) 的高频历史输出请求，用于算反应谱和傅里叶谱
        assembly = model.rootAssembly
        if inst_name in assembly.instances:
            instance = assembly.instances[inst_name]
            if 'TOP_SURFACE' in instance.sets:
                region = instance.sets['TOP_SURFACE']
                model.HistoryOutputRequest(
                    name='H-Output-TopSurface',
                    createStepName=step_name,
                    region=region,
                    variables=('U1', 'U2', 'V1', 'V2', 'A1', 'A2'),
                    frequency=1)
                log_step(logger, '%s 模型已成功为 %s 实例的 TOP_SURFACE 节点集创建高频历史输出', new_model_name, inst_name)
...
def _load_case_config(material_cfg, geometry_cfg, mesh_size, damping_cfg, logger):  # 加载工况配置注入
    ...
    try:  # 尝试读取并覆盖
        ...
        if isinstance(cfg.get('damping_cfg'), dict):  # 提供了阻尼覆盖
            damping_cfg = _deep_merge(damping_cfg, cfg['damping_cfg'])  # 合并阻尼配置
        if logger:  # 记录成功
            log_step(logger, '已加载 case_config.json 覆盖默认配置: 入射角=%s, 层数(有限层)=%d, i=%s, 阻尼=%s',  # 输出关键覆盖项
                     material_cfg.get('angle'), len(material_cfg.get('layers', [])), geometry_cfg.get('i'), damping_cfg.get('enable'))
...
def _write_case_meta(material_cfg, geom, site, mesh_size, damping, script_name, logger):  # 写出统一工况元数据
    ...
    try:  # 元数据写出不应影响建模主流程
        ...
        # 计算并组装阻尼的元数据
        strat = _build_stratigraphy(site, geom)
        layers_damping = []
        for idx, band in enumerate(strat):
            mat = band['mat']
            is_bedrock = (idx == 0)
            q_val, xi_val = _damping_ratio_from_q(mat.cs, is_bedrock, damping)
            a_val, b_val = _rayleigh_coeffs(xi_val, damping, damping['fc'])
            layers_damping.append({
                'name': band['name'],
                'cs': _meta_f(mat.cs),
                'Q': _meta_f(q_val),
                'xi': _meta_f(xi_val),
        ...
        damping_meta = {
            'enable': damping.get('enable', False),
            'method': damping.get('method', 'rayleigh'),
            'fc': _meta_f(damping.get('fc')),
            'qs_factor': _meta_f(damping.get('qs_factor')),
            'q_bedrock': _meta_f(damping.get('q_bedrock')),
            'f1_factor': _meta_f(damping.get('f1_factor')),
            'f2_factor': _meta_f(damping.get('f2_factor')),
            'layers': layers_damping
        }
        ...
        meta = {  # 组装规范元数据（嵌套结构，供下游通用展放）
            ...
            'damping': damping_meta,  # 阻尼配置与换算结果
...
def main():
    ...
    global material_cfg, geometry_cfg, mesh_size, damping_cfg  # 声明为全局以便用注入配置整体覆盖（所有 callee 均按值取用，安全）
    ...
    try:
        ...
        material_cfg, geometry_cfg, mesh_size, damping_cfg = _load_case_config(material_cfg, geometry_cfg, mesh_size, damping_cfg, logger)  # 加载并覆盖
        ...
        fc_est = 0.0
        if acc_info:
            first_acc_file = acc_info[0][0]
            try:
                acc_data = np.loadtxt(first_acc_file)
                if acc_data.ndim == 2 and acc_data.shape[0] >= 2:
                    t_arr = acc_data[:, 0]
                    a_arr = acc_data[:, 1]
                    dt_acc = t_arr[1] - t_arr[0]
                    fc_est = _estimate_dominant_freq(a_arr, dt_acc)
                    log_step(logger, '从 %s 自动估计主频 fc = %.4f Hz', first_acc_file, fc_est)
            except Exception as e:
                log_step(logger, '从加速度记录估计主频失败: %s', str(e))

        damping = _resolve_damping(damping_cfg, fc_est)

        _write_case_meta(material_cfg, geom, site, mesh_size, damping, _script_name(), logger)  # 写出统一工况元数据 case_meta.json（脚本名不依赖 __file__）
        ...
        base_model, part_name, inst_name = create_model(  # 创建斜坡基础几何与网格模型
            site=site, geom=geom, mesh_size=mesh_size, cae_name=cae_name, damping=damping, logger=logger)  # 传入场地/几何/网格/文件名/阻尼

        flat_base_model, flat_part_name, flat_inst_name = create_flat_model(  # 创建平坦自由场基础模型
            site=site, geom=geom, mesh_size=mesh_size, damping=damping, logger=logger)  # 传入场地/几何/网格/阻尼
