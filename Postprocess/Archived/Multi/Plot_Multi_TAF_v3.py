# -*- coding: utf-8 -*-
"""复现论文【图8】风格的跨工况 TAF 对比图（v3：像图15 v3 一样分别输出图 (a) 和 (b) 两个独立的 2×2 合成子图与子面板）。

论文图8：基岩-覆盖层双层模型，阻抗比 Vr/Vs2=1.25、覆盖层厚度比 h/H=0.50 的典型工况。
  本脚本拆分为两个坡角的独立 2×2 图像输出（与 Fig15_compare_v3 行为模式完全一致）：
    (a) 缓边坡 i=30° 合成图
    (b) 陡边坡 i=60° 合成图
  每个合成图内部是 2×2 子图矩阵：
...
数据来源（仅集中 results/ 模式，与 Plot_Fig15_compare_v3 同口径）：
  先用 Collect_TAF_results_v2.py 把各工况 CSV 汇到 results/，本脚本读取：
  ...
  坡角 i、入射角 θs 直接取自 index.csv 规范列；无量纲频率 a0 由 record 主频 fc(Hz) 与该工况 a0_base
    按 a0 = fc·a0_base 计算（a0_base=2·(H−h)/Vs2，逐工况随 case_meta 给定）。

运行：
  先 `python Postprocess/Collect_TAF_results_v2.py <工况根目录>` 生成 results/index.csv，再
  `python Postprocess/Plot_Multi_TAF_v3.py <工况根目录>`（自动找其下 results/）
  或直接 `python Postprocess/Plot_Multi_TAF_v3.py <results 目录>`；不传参数取当前目录。
"""
...
# ==============================================================================
#  绘图（新图8 风格排版：每个坡角 i 绘制一张独立的 2×2 合成图，行=入射角，列=H/V，曲线=a0）
# ==============================================================================
...
def plot_fig8_one_angle(i_ang, cases, x_crest, x_toe, total_L, out_dir, part_tag):  # 绘制单个坡角的 2×2 合成子图 + 拆分子面板
    """对某一坡角 i_ang 绘制合成图：行=入射角 θs（升序），列=H/V；面板内按 a0 画曲线。

    合成图与各子图统一输出到 out_dir/Fig8_i{i}/（子图在其下 panels/），见 save_composite_with_panels。
    """
    thetas = sorted({c['theta'] for c in cases})  # 出现的入射角（升序，决定组内行顺序）
    n_theta = max(1, len(thetas))  # 行数 (入射角数)
    n_col = len(COLUMNS)  # 列数 (H/V)
    total_rows = n_theta  # 总行数
    ctx = {'cases': cases, 'x_crest': x_crest, 'x_toe': x_toe, 'total_L': total_L}  # 绘制上下文
    
    # ---- 合成图：总网格 ----
    comp_size = (3.15 * n_col, max(2.4, 2.3 * total_rows))  # 双栏宽，高随行数变化
    fig, axes = plt.subplots(total_rows, n_col, figsize=comp_size, squeeze=False)  # 创建网格
    
    for ti, theta in enumerate(thetas):  # 遍历行（入射角）
        for ci, (key, ylabel, _fixed) in enumerate(COLUMNS):  # 遍历列（H/V 分量）
            draw_taf_panel(axes[ti][ci], i_ang, theta, key, ylabel,  # 画该面板
                           ylim_for(i_ang, key, cases), ctx,
                           title=r'$\theta_s = %g^\circ$' % theta, show_xlabel=True)
            
    fig.tight_layout(rect=(0, 0, 1, 1), h_pad=0.5)  # 紧凑布局
    
    # ---- 在底部居中标注坡角子标签 (a) i=30° 或 (b) i=60° ----
    last_row = total_rows - 1  # 末行行号
    pos = axes[last_row][0].get_position()  # 末行左面板位置
    letter = part_tag if part_tag else 'a'  # 子标签
    fig.text(pos.x0 + pos.width * n_col / 2 + 0.04, pos.y0 - 0.04,
             r'(%s)  $i=%g^\circ$' % (letter, i_ang),
             fontsize=9, va='top', ha='center', fontweight='bold')  # 居中加粗标签
    ...
    for theta in thetas:  # 遍历入射角
        for ci, (key, ylabel, _fixed) in enumerate(COLUMNS):  # 遍历分量
            ...
            name = 'i%g_ths%g_TAF-%s' % (i_ang, theta, comp)  # 语义子图名（如 i30_ths0_TAF-H）
            draw_fn = functools.partial(draw_taf_panel, i_ang=i_ang, theta=theta, key=key,
    ...
    fig_name = 'Fig8_i%g' % i_ang  # 合成图名（按坡角区分）
    res = save_composite_with_panels(out_dir, fig_name, fig, panel_specs,
                                     composite_size=comp_size, panel_size=(3.15, 2.0))
    print('  合成图: %s' % '; '.join(os.path.basename(p) for p in res['composite']))
    print('  子图 %d 张 -> %s' % (len(res['panels']), os.path.join(res['dir'], 'panels')))
...
# ==============================================================================
def main():  # 主入口
    print('>>> 启动图8 风格 TAF 对比绘图 (Plot_Multi_TAF_v3)...')  # 启动提示
    ...
    if data_dir is None:  # 未找到 index.csv
        print('错误：未找到 index.csv（集中结果）。'
              '\n请先运行 Collect_TAF_results_v2.py 生成 results/index.csv，'
    ...
    slope_angles = sorted({c['slope_i'] for c in cases if int(round(c['slope_i'])) in YLIM_BY_I})
    n_i = len(slope_angles)
    n_t = len({c['theta'] for c in cases})
    n_a = len({c['a0'] for c in cases})
    print('>>> 共 %d 个工况：筛选坡角 %d 档 = %s、入射角 %d 档、a0 %d 档；按图8 风格分别出图。' % 
          (len(cases), n_i, slope_angles, n_t, n_a))
          
    out_dir = os.path.join(data_dir, 'Fig8_TAF_compare')  # 统一输出到 results/Fig8_TAF_compare/
    part_letters = ['a', 'b', 'c', 'd']
    for idx, i_ang in enumerate(slope_angles):
        sub = [c for c in cases if abs(c['slope_i'] - i_ang) < 1e-6]
        part_tag = part_letters[idx] if idx < len(part_letters) else str(idx + 1)
        print('--- 坡角 i=%g°：%d 个工况 ---' % (i_ang, len(sub)))
        plot_fig8_one_angle(i_ang, sub, x_crest, x_toe, total_L, out_dir, part_tag)
    print('>>> 图8 风格对比图全部完成，输出目录：%s' % out_dir)  # 完成提示
