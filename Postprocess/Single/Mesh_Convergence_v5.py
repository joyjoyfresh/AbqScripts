...
FOLDER_PATTERN = re.compile(r'^fuke-30-(\d+)$')  # 文件夹名匹配
OUTPUT_DIR = os.path.join(ROOT_DIR, 'Comparison-30')  # 统一输出目录
...
def scan_folders(root_dir):
    """扫描 root_dir 下的 fuke-?-* 文件夹，返回 [(mesh_size, folder_path), ...] 按 mesh_size 排序"""
    results = []
...
def get_folder_size_bytes(folder_path):
    """递归统计单个文件夹总大小（字节）"""
    total_size = 0  # 初始化目录总字节数
    for current_root, _, files in os.walk(folder_path):  # 递归遍历目录树中的所有文件
        for filename in files:  # 遍历当前目录下的文件名
            file_path = os.path.join(current_root, filename)  # 拼接当前文件的完整路径
            if os.path.isfile(file_path):  # 仅对真实文件进行大小累加
                total_size += os.path.getsize(file_path)  # 累加当前文件字节数
    return total_size  # 返回目录总字节数


def collect_folder_size_by_mesh(folders):
    """收集各 mesh_size 对应模型文件夹总大小（字节）"""
    folder_size_by_mesh = {}  # 初始化文件夹大小字典
    for mesh_size, folder_path in folders:  # 遍历所有模型目录
        folder_size_by_mesh[mesh_size] = get_folder_size_bytes(folder_path)  # 统计并保存当前模型目录大小
    return folder_size_by_mesh  # 返回模型目录大小字典
...
def main():
    ...
    if not folders:
        print('错误: 在 {} 下未找到 fuke-?-* 文件夹'.format(ROOT_DIR))
        sys.exit(1)
    ...
    # 1.2 读取各模型目录大小数据
    folder_size_by_mesh = collect_folder_size_by_mesh(folders)
    if folder_size_by_mesh:
        print('已统计 {} 个模型的目录大小数据'.format(len(folder_size_by_mesh)))
    else:
        print('警告: 未统计到任何目录大小数据，将跳过目录大小绘图')
    ...
    # 4. 计算误差并输出 CSV
    output_dir = OUTPUT_DIR  # 使用统一输出目录
    if not os.path.isdir(output_dir):  # 若输出目录不存在则自动创建
        os.makedirs(output_dir)  # 创建输出目录用于集中保存结果文件
    print('输出目录: {}'.format(output_dir))  # 打印输出目录路径便于定位结果文件
    ...
    # --- 图4: 各模型脚本总运行时间 + 文件夹大小对比 ---
    if runtime_by_mesh or folder_size_by_mesh:
        fig4, ax4 = plt.subplots(figsize=(10, 6))
        fig4.suptitle('各网格精度模型运行时间与目录大小对比', fontsize=14, fontweight='bold')
        ms_runtime = sorted(set(runtime_by_mesh.keys()) | set(folder_size_by_mesh.keys()))
        rt_runtime = [runtime_by_mesh.get(ms, 0.0) for ms in ms_runtime]
        size_gb = [folder_size_by_mesh.get(ms, 0.0) / (1024.0 ** 3) for ms in ms_runtime]

        # 导出图4对应数据为 CSV，便于论文制图复用与数据追溯
        fig4_csv_path = os.path.join(output_dir, 'mesh_convergence_runtime_and_size.csv')  # 设置图4数据 CSV 输出路径
        with open(fig4_csv_path, 'w', newline='') as f4_csv:  # 以写入模式创建图4数据 CSV 文件
            writer4 = csv.writer(f4_csv)  # 创建图4数据 CSV 写入器
            writer4.writerow(['mesh_size', 'runtime_s', 'folder_size_gb'])  # 写入图4数据表头
            for ms, runtime_s, folder_size_gb in zip(ms_runtime, rt_runtime, size_gb):  # 逐行写入图4每个网格尺寸对应的数据
                writer4.writerow([ms, '{:.6f}'.format(runtime_s), '{:.6f}'.format(folder_size_gb)])  # 写入网格尺寸、运行时间与目录大小
        print('图4数据 CSV 已保存: {}'.format(fig4_csv_path))  # 输出图4数据 CSV 保存路径

        bars = ax4.bar(ms_runtime, rt_runtime, color='#4C72B0', edgecolor='black', linewidth=0.9,
                       label='运行时间')
        ax4.set_xlabel('网格尺寸 (m)', fontsize=12)
        ax4.set_ylabel('脚本总运行时间 (s)', fontsize=12, color='#4C72B0')
        ax4.tick_params(axis='y', labelcolor='#4C72B0')
        ax4.set_xticks(ms_runtime)
        style_axis(ax4)

        ax4b = ax4.twinx()
        line_size, = ax4b.plot(ms_runtime, size_gb, color='#DD8452', marker='o', linestyle='-',
                               linewidth=2.0, markersize=6, label='目录大小')
        ax4b.set_ylabel('模型目录大小 (GB)', fontsize=12, color='#DD8452')
        ...
        for x_pos, value in zip(ms_runtime, size_gb):
            ax4b.text(x_pos, value, '{:.2f}'.format(value),
                      ha='center', va='bottom', fontsize=9, color='#DD8452')

        handles_1, labels_1 = ax4.get_legend_handles_labels()
        handles_2, labels_2 = [line_size], ['目录大小']
        ax4.legend(handles_1 + handles_2, labels_1 + labels_2,
                   frameon=True, fancybox=False, edgecolor='black', loc='upper left')

        fig4.tight_layout(rect=[0, 0, 1, 0.95])
        fig4_path = os.path.join(output_dir, 'mesh_convergence_runtime_and_size.png')
        fig4.savefig(fig4_path, dpi=200, bbox_inches='tight')
        print('运行时间与目录大小对比图已保存: {}'.format(fig4_path))
