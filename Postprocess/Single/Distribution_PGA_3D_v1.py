...
def style_3d_axes(ax, angle_value, h_values, i_values, z_limits):  # 定义 3D 坐标轴样式设置函数
    ...
    ax.set_zlabel(r'$PGA_{h,\max}\,(g)$', labelpad=4, fontsize=10)  # 设置 Z 轴标签
    x_min = min(0.0, float(np.min(h_values))) if len(h_values) > 0 else 0.0  # 计算 X 轴下界并尽量包含 0
    x_max = float(np.max(h_values)) if len(h_values) > 0 else 1.0  # 计算 X 轴上界
    y_min = float(np.min(i_values)) if len(i_values) > 0 else 0.0  # 计算 Y 轴下界
    y_max = float(np.max(i_values)) if len(i_values) > 0 else 1.0  # 计算 Y 轴上界
    if abs(x_max - x_min) < 1e-9:  # 判断 X 轴是否退化为单值
        x_min = x_min - 1.0  # 为退化坐标扩展下界
        x_max = x_max + 1.0  # 为退化坐标扩展上界
    if abs(y_max - y_min) < 1e-9:  # 判断 Y 轴是否退化为单值
        y_min = y_min - 1.0  # 为退化坐标扩展下界
        y_max = y_max + 1.0  # 为退化坐标扩展上界
    ax.set_xlim(x_min, x_max)  # 设置 X 轴范围
    ax.set_ylim(y_max, y_min)  # 设置 Y 轴反向范围以贴近示例视角
    ax.set_zlim(z_limits[0], z_limits[1])  # 设置 Z 轴范围
    x_ticks = choose_ticks(h_values, 3)  # 生成 X 轴刻度
    y_ticks = choose_ticks(i_values, 3)  # 生成 Y 轴刻度
    if x_ticks:  # 判断 X 轴刻度列表是否非空
        ax.set_xticks(x_ticks)  # 写入 X 轴刻度
        ax.set_xticklabels([format_number_text(v) for v in x_ticks], fontsize=9)  # 设置 X 轴刻度文本
    if y_ticks:  # 判断 Y 轴刻度列表是否非空
        ax.set_yticks(y_ticks)  # 写入 Y 轴刻度
        ax.set_yticklabels([format_number_text(v) for v in y_ticks], fontsize=9)  # 设置 Y 轴刻度文本
    z_ticks = np.linspace(z_limits[0], z_limits[1], 3)  # 生成 Z 轴三段刻度
    ax.set_zticks(z_ticks)  # 写入 Z 轴刻度
    ax.set_zticklabels([f'{v:.2f}'.rstrip('0').rstrip('.') for v in z_ticks], fontsize=9)  # 设置 Z 轴刻度文本
