...
"""根据现有 PGA 数据生成按斜坡高度 h 分面的最大谱加速度 Sa 图表"""  # 说明脚本用途
...
def load_pga_dataframe(filepath, target_cols):  # 定义读取 PGA 文件的函数
    """读取 PGA CSV 并校验列"""  # 函数说明
...
def extract_max_sa_from_pga(pga_df):  # 定义从 PGA 数据提取最大 Sa 的函数
    """从 PGA 数据表提取顶面最大谱加速度（取水平分量绝对值的最大值）"""  # 函数说明
    if pga_df.empty:  # 判断输入是否为空
        return None  # 返回空值表示无效数据
    
    # 计算水平分量的最大值
    if 'PGA_h' in pga_df.columns:  # 判断是否包含水平加速度列
        pga_h_values = pd.to_numeric(pga_df['PGA_h'], errors='coerce').to_numpy(dtype=float)  # 转换为浮点数组
        pga_h_max = float(np.max(np.abs(pga_h_values)))  # 取绝对值的最大值
    else:  # 若无水平加速度列
        pga_h_max = 0.0  # 设为 0
    
    return pga_h_max  # 返回最大谱加速度


def collect_records(batch_root):  # 定义扫描目录并收集记录的函数
    """扫描工况目录并从 PGA 文件采集记录"""  # 函数说明
    records = []  # 初始化记录列表
    ...
    for entry_name in sorted(os.listdir(batch_root)):  # 遍历根目录下全部条目
        ...
        for file_name in sorted(os.listdir(folder_path)):  # 遍历目录内文件名
            ...
            pga_files.append(os.path.join(folder_path, file_name))  # 记录 PGA 文件路径
        
        # 处理每个 PGA 文件
        for csv_path in pga_files:  # 遍历每个 PGA 文件
            ...
            try:  # 尝试读取 PGA 数据
                pga_df = load_pga_dataframe(csv_path, TARGET_COLUMNS)  # 读取 PGA 数据
                sa_max = extract_max_sa_from_pga(pga_df)  # 提取最大 Sa
    ...
    if not records:  # 判断是否采集到有效记录
        raise FileNotFoundError('未采集到有效 PGA 数据，请检查目录与文件命名。')  # 抛出无数据异常
