...
CURVE_STYLES = [
    ...
    {'color': '#984ea3', 'linestyle': ':', 'linewidth': 1.5},   # 第四条曲线: 紫色细虚线
]
...
def load_pga_file(filepath):
    ...
    df = df[['node_label', 'x', 'y', 'PGA_h', 'PGA_v']].copy()
...
def main():
    ...
    for base_key, paths in sorted(valid_pairs.items()):
        ...
        try:
            ...
            slope_x = slope_df['x'].to_numpy()
            flat_x = flat_df['x'].to_numpy()
            
            flat_pga_h_interp = np.interp(slope_x, flat_x, flat_df['PGA_h'].to_numpy())
            flat_pga_v_interp = np.interp(slope_x, flat_x, flat_df['PGA_v'].to_numpy())
            ...
            taf_h[mask_h] = slope_df['PGA_h'].to_numpy()[mask_h] / flat_pga_h_interp[mask_h]
            taf_v[mask_v] = slope_df['PGA_v'].to_numpy()[mask_v] / flat_pga_v_interp[mask_v]
            
            # 生成 TAF 数据表并保存
            taf_df = pd.DataFrame({
                'node_label': slope_df['node_label'],
                'x': slope_x,
                'y': slope_df['y'],
                'TAF_h': taf_h,
                'TAF_v': taf_v
