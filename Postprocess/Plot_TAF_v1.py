...
"""
真实坐标 TAF 曲线绘制脚本
从已生成的 TAF-*.csv 文件中读取数据并绘制对比图。

主要特点:
1. 仅保留从 TAF-*.csv 文件读取并绘图的功能，不再进行 PGA 提取与 TAF 计算。
2. 横坐标为节点的真实绝对坐标 x。
3. 坡顶与坡脚的真实坐标 (#1 与 #2) 优先从同目录下的 PGA-*-slope.csv 提取，若不存在则使用预设默认值。
...
def main():
    ...
    if not taf_records:
        print("未加载到任何有效 TAF 数据记录，绘图终止。")
        return
        
    # 4. 按 base_motion 进行分组，并在同一图表中绘制不同频率的对比图
