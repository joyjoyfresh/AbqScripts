# -*- coding: utf-8 -*-
"""生成G1r使用的两条0.3g、1 ms真实波输入并写出可追溯清单。"""
...
TARGET_PGA = 0.3 * 9.81
LOWPASS_HZ = 12.0
STOPBAND_CHECK_HZ = 15.0
FILTER_ORDER = 6
TAPER_SECONDS = 0.50
VELOCITY_TREND_LIMIT = 1.0e-8
HIGH_FREQUENCY_ENERGY_LIMIT = 1.0e-4
SOURCES = (
    ('el_centro', os.path.join(REPO_ROOT, 'Wave', 'Seismic', 'Scaled', 'El_Centro_scaled.txt')),
    ('loma_prieta', os.path.join(REPO_ROOT, 'Wave', 'Seismic', 'Scaled', 'Loma_Prieta_scaled.txt')),
)
...
def main():
    ...
    manifest = {
        'schema_version': 1,
        'purpose': 'G1r flat-field reference validation and real-wave closure inputs',
    ...
    manifest_path = os.path.join(OUTPUT_DIR, 'g1r_real_input_manifest.json')
